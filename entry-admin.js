...
function unlock(){sessionStorage.setItem('apdcEntryAdmin','yes');sessionStorage.setItem('apdcDeviceInfo',deviceInfo());$('loginBox').classList.add('hidden');$('adminBox').classList.remove('hidden');$('sessionInfo').textContent=`Device: ${deviceInfo()}`;loadAll()}
...
if(sessionStorage.getItem('apdcEntryAdmin')==='yes'){ unlock(); }
...
$('saveAll').onclick=async()=>{commitEditor();const players=flattenGroups(groups).filter(x=>x.backNo&&x.competitor&&x.event&&x.section);const worker='Admin';const device=sessionStorage.getItem('apdcDeviceInfo')||deviceInfo();status('Saving entries and creating restore point…');const currentSnap=await get(ref(db,'apdcPublic/players'));const current=currentSnap.exists()&&Array.isArray(currentSnap.val())?currentSnap.val():[];const versionsSnap=await get(ref(db,'apdcPublic/entryVersions'));let versions=versionsSnap.exists()&&Array.isArray(versionsSnap.val())?versionsSnap.val():[];versions.unshift({id:Date.now(),createdAt:new Date().toISOString(),worker,device,records:current.length,players:current});versions=versions.slice(0,10);await set(ref(db,'apdcPublic/entryVersions'),versions);await set(ref(db,'apdcPublic/players'),players);await set(ref(db,'apdcPublic/meta'),{updatedAt:new Date().toISOString(),updatedBy:worker,device});status(`Saved ${players.length} entry records. Restore point created (${versions.length}/10).`)};
