...
const restGet=async(path)=>{const snap=await get(ref(firebaseDb,`apdcPublic/${path}`));return snap.exists()?snap.val():null};
const restPut=async(path,value)=>{await set(ref(firebaseDb,`apdcPublic/${path}`),value);return value};
const rootGet=async(path)=>{const snap=await get(ref(firebaseDb,path));return snap.exists()?snap.val():null};
const rootPut=async(path,value)=>{await set(ref(firebaseDb,path),value);return value};
...
function availableBackNumbers(limit=12){const used=usedBackNumbers(selected);const current=selected>=0?String(groups[selected]?.backNo||'').trim():'';const out=[];for(let n=1;n<=299&&out.length<limit;n++){const key=String(n);if(key!==current&&!used.has(key))out.push(n)}return out}
function renderBackNoStatus(){const box=$('backNoStatus');if(!box)return;const raw=String($('backNo').value||'').trim();const used=usedBackNumbers(selected);const current=selected>=0?String(groups[selected]?.backNo||'').trim():'';let msg='';let cls='backno-neutral';if(raw){if(!/^\d+$/.test(raw)||Number(raw)<1||Number(raw)>299){msg='Back No. must be between 1 and 299.';cls='backno-bad'}else if(raw===current){msg=`CURRENT BACK NO. ${esc(raw)}`;cls='backno-good'}else if(used.has(raw)){msg=`NO. ${esc(raw)} is already used by ${esc(used.get(raw))}.`;cls='backno-bad'}else{msg=`NO. ${esc(raw)} is available.`;cls='backno-good'}}else{msg='Choose an unused Back No.'}const avail=availableBackNumbers().map(n=>`<button type="button" class="backno-chip" data-backno="${n}">${n}</button>`).join('');box.className=`backno-status ${cls}`;box.innerHTML=`<div>${msg}</div><div class="backno-available-label">AVAILABLE</div><div class="backno-chips">${avail}</div>`;box.querySelectorAll('[data-backno]').forEach(b=>b.onclick=()=>{$('backNo').value=b.dataset.backno;renderBackNoStatus()})}
...
async function syncJudgeTimetable(players){
  // TIMETABLE LOCK 2026-07-27: entry edits must never rewrite timetable structure/order/time.
  const source=await fetchJson('timetable-data.json');
...
function eventDisplayTitle(e){const candidates=[e.event,e.section,e.division].map(v=>String(v||'').trim()).filter(Boolean);const score=v=>{let n=v.length;if(/\b(?:C|S|R|J|W|T|F|Q|V|CR|RJ|CRJ|CRS|CSRJ|WTQ)\b/i.test(v))n+=80;if(/\b(?:dance|latin|standard|modern|formation)\b/i.test(v))n+=40;if(/\b(?:under|over)\s*\d+/i.test(v))n+=30;return n};return candidates.sort((a,b)=>score(b)-score(a))[0]||String(e.event||'').trim()}
function eventDisplayMeta(e,title){const titleLower=String(title||'').toLowerCase();const vals=[e.style,e.entryType||e.division].map(v=>String(v||'').trim()).filter(Boolean);const out=[];for(const v of vals){const low=v.toLowerCase();if(titleLower.includes(low))continue;if(out.some(x=>x.toLowerCase()===low))continue;if(/^(solo|couple)$/i.test(v)){out.push(v);continue}if(!titleLower.includes(low))out.push(v)}return out.join(' · ')}
...
async function saveOnlineMaster(reason='SAVE'){
  ...
  if(conflicts.length){renderBackNoStatus();throw new Error('Back No. problem: '+conflicts.join(' / '));}
  const players=flattenGroups(groups).filter(x=>x.backNo&&x.competitor&&x.event&&x.section);
  const device=sessionStorage.getItem('apdcDeviceInfo')||deviceInfo();
  const current=await restGet('players');
  if(!Array.isArray(current))throw new Error('ONLINE MASTER could not be verified before save');
  let versions=[];
  try{const v=await restGet('entryVersions');if(Array.isArray(v))versions=v}catch{}
  versions.unshift({id:Date.now(),createdAt:new Date().toISOString(),device,reason,records:current.length,players:current});
  versions=versions.slice(0,10);
  await restPut('entryVersions',versions);
  await restPut('players',players);
  const syncedTimetable=await syncJudgeTimetable(players);
  await restPut('meta',{updatedAt:new Date().toISOString(),device,lastAction:reason,timetableEntrySyncAt:new Date().toISOString()});
  return {players,versions,syncedTimetable};
}
function renderEditor(){const editor=$('playerEditor');if(selected<0||!groups[selected]){editor.classList.add('hidden');$('backNo').value='';$('competitor').value='';$('entryRows').innerHTML='';if($('backNoStatus'))$('backNoStatus').innerHTML='';return}editor.classList.remove('hidden');const g=groups[selected];$('editorName').textContent=g.competitor||'NEW PLAYER';$('backNo').value=g.backNo;$('competitor').value=g.competitor;renderBackNoStatus();$('entryRows').innerHTML=g.entries.map(rowHtml).join('')||'<p class="empty-note">No entries. Press ADD ENTRY and select a saved section.</p>';$('entryRows').querySelectorAll('.entry-picker-row').forEach(row=>{row.querySelector('.event-no-input').onclick=()=>openMenu(row);row.querySelector('.event-open').onclick=()=>openMenu(row);row.querySelectorAll('.picker-trigger-input').forEach(inp=>inp.onclick=()=>openMenu(row))});$('entryRows').querySelectorAll('[data-del]').forEach(b=>b.onclick=async()=>{if(!masterReady){status('DELETE BLOCKED: ONLINE MASTER is not connected.');return}commitEditor();const idx=Number(b.dataset.del);const entry=groups[selected]?.entries[idx];if(!entry)return;const label=entry.event||entry.section||`Event ${entry.eventNo||''}`;if(!confirm(`Delete ${label} from this player?\n\nThis will update the live website immediately.`))return;const backup={...entry};const playerIndex=selected;groups[playerIndex].entries.splice(idx,1);renderEditor();renderList();status(`Deleting ${label} from ONLINE MASTER…`);try{const result=await saveOnlineMaster(`DELETE ENTRY: ${groups[playerIndex].backNo} ${groups[playerIndex].competitor} · ${label}`);status(`Deleted ${label}. LIVE Search is updated. Restore points: ${result.versions.length}/10.`)}catch(err){console.error(err);groups[playerIndex].entries.splice(idx,0,backup);renderEditor();renderList();status(`DELETE FAILED: ${err?.message||err}. Nothing was removed online.`)}})}
...
$('filter').oninput=renderList;$('backNo').oninput=()=>{if(selected>=0){groups[selected].backNo=$('backNo').value;renderBackNoStatus();renderList()}};$('competitor').oninput=()=>{if(selected>=0){groups[selected].competitor=$('competitor').value;$('editorName').textContent=$('competitor').value||'NEW PLAYER';renderList()}};$('closeEditor').onclick=closePlayer;$('newPlayer').onclick=()=>{commitEditor();groups.push({backNo:'',competitor:'',entries:[]});openPlayer(groups.length-1)};$('addEntry').onclick=()=>{if(selected<0)return;commitEditor();groups[selected].entries.push({eventNo:'',event:'',section:'',style:'',division:'',entryType:''});renderEditor();setTimeout(()=>{const rows=$('entryRows').querySelectorAll('.entry-row');if(rows.length)openMenu(rows[rows.length-1])},0)};$('deletePlayer').onclick=async()=>{if(selected<0||!masterReady)return;commitEditor();const idx=selected;const g=groups[idx];if(!confirm(`Delete NO. ${g.backNo} ${g.competitor} and ALL ${g.entries.length} entries?\n\nThis will update the live website immediately.`))return;const backup=JSON.parse(JSON.stringify(g));groups.splice(idx,1);selected=-1;renderList();renderEditor();status(`Deleting NO. ${backup.backNo} ${backup.competitor} from ONLINE MASTER…`);try{const result=await saveOnlineMaster(`DELETE PLAYER: ${backup.backNo} ${backup.competitor}`);status(`Deleted NO. ${backup.backNo} ${backup.competitor}. LIVE Search is updated. Restore points: ${result.versions.length}/10.`)}catch(err){console.error(err);groups.splice(idx,0,backup);renderList();status(`DELETE FAILED: ${err?.message||err}. Nothing was removed online.`)}};$('loadBase').onclick=loadAll;
$('saveAll').onclick=async()=>{if(!masterReady){status('SAVE BLOCKED: ONLINE MASTER is not connected. Reload and try again.');return}status('Saving entries and creating restore point…');try{const result=await saveOnlineMaster('SAVE ENTRIES');status(`Saved ${result.players.length} entry records. LIVE Search is updated. Timetable order/time remained locked (${result.syncedTimetable.entrySyncChanges||0} local count checks). Restore points: ${result.versions.length}/10.`)}catch(err){console.error(err);if(String(err.message||'').startsWith('Back No. problem:'))alert(err.message.replace('Back No. problem: ','Back No. problem:\n\n').replace(/ \/ /g,'\n'));status(`SAVE FAILED: ${err?.message||err}`)}};
